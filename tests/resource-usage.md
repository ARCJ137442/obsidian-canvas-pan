...
| 2026-08-26 | `pan-mobile-api-telemetry-tests-v1` | — | 23/23，退出码 0 | 2273.16 ms | 341,319,680 B（325.51 MiB） | 4.172 s | 2 |
| 2026-08-26 | `pan-mobile-api-telemetry-build-v1` | — | 成功，退出码 0；短进程采样数 1，峰值可能漏采 | 1868.78 ms | 0 B（短进程可能漏采） | 0 s（短进程可能漏采） | 1 |
| 2026-08-26 | `pan-mobile-pointer-lease-tests-v1` | — | 失败：TypeScript 测试类型未同步 | 2161.12 ms | 324,419,584 B（309.39 MiB） | 3.922 s | 2 |
| 2026-08-26 | `pan-mobile-pointer-lease-tests-v2` | — | 25/25，退出码 0 | 2937.46 ms | 318,169,088 B（303.43 MiB） | 4.031 s | 2 |
| 2026-08-26 | `pan-mobile-pointer-lease-build-v1` | — | 成功，退出码 0；短进程采样数 1，峰值可能漏采 | 1509.54 ms | 0 B（短进程可能漏采） | 0 s（短进程可能漏采） | 1 |
| 2026-08-26 | `canvas-pan-1.0.10-release-tests` | `npm test` | 25/25，退出码 0；1.0.10 发布验证 | 2573.21 ms | 324,374,528 B（309.35 MiB） | 4.938 s | 2 |
| 2026-08-26 | `canvas-pan-1.0.10-release-build` | `npm run build` | 成功，退出码 0；1.0.10 发布验证，短进程采样数 1 | 1773.03 ms | 0 B（短进程可能漏采） | 0 s（短进程可能漏采） | 1 |

采样器使用仓库内的 [`measure-command.ps1`](../scripts/measure-command.ps1)，不再依赖相邻源码仓库。工作集和 CPU 仅代表被测命令进程树，不等同于整台机器的总资源占用；短命令可能因采样间隔漏掉瞬时峰值。
